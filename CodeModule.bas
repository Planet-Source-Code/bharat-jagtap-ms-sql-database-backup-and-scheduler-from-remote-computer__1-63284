...
Public strBackupFileName As String
Public g_strBackupPath As String
Public bTakeBackup As Boolean
Public strServerWindowsName As String
Public gsBackUpVariable As Integer
...
Public gdbConnection As New ADODB.Connection
'-- C Style argv
...
Public Function UZDLLPass(ByRef p As UNZIPCBCh, _
  ...
  If gsBackUpVariable = 2 Then
       szpassword = "Critical%data@very*userfull~05"
  ElseIf gsBackUpVariable = 3 Then
       szpassword = "This*will!save&my@work^05"
  End If
   ...
   ' gdbConnection.Close
    ...
    If gdbConnection.State = adStateOpen Then gdbConnection.Close
        ...
        If Len(Trim(g_sPassword)) <> 0 Then
              gdbConnection.Open "Provider=SQLOLEDB.1;Password='" & g_sPassword & "';Persist Security Info=True;User ID=sa;Initial Catalog=MASTER;Data Source=" & g_sServerName
        Else
...
Private Function getFreeSpaceOnDriveInMB(Optional ByVal strDriveLetter As String = "C") As String
On Error GoTo ErrHand
    ...
    'MsgBox "Capacity: " & CDec(oDrive.TotalSize) / (varMB) & " MB"
    'MsgBox "Free: " & Format(CDec(oDrive.FreeSpace) / (varMB)) & " MB"
...
Public Sub IsApplicationAlreadyInstenciated()
On Error GoTo ErrHand
    If App.PrevInstance Then
        MsgBox "Another Instance Of This Application Is Running..", vbInformation
...
Sub Main()
On Error GoTo ErrHand:
    If UCase(App.EXEName) = "JBClient" Then
        FrmServer.Show
    Else
        frmLogin.Show
    End If
